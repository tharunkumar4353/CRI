...
export class CycleComponent implements OnInit {
  ...
  operatorName: string;
  processType: string;
  cnc01Data: any = {}; // To store data specifically for CNC-02
  id: any;
  customid: any;
runtime: any;
private _empAcceptCount: number = 0;
  custcode: any;
  fgname: any;


  // Use a getter and setter for empAcceptCount
  get empAcceptCount(): number {
    return this._empAcceptCount;
  }

  set empAcceptCount(value: number) {
    this._empAcceptCount = value;
    // Update runtime calculation when empAcceptCount is set
    this.runtime = this.cycle * this._empAcceptCount;
  }
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe(params => {
      ...
      this.machinename = params['machinename'] || 'CNC-01';
      ...
      console.log('custom id:', this.customid);
    ...
    this.socketService.getLiveData().subscribe((data: any) => {
      ...
      console.log('Real-time CNC-01 data:', this.cnc01Data);
    });
  }
  ...
  saveEmpAcceptCount(customid: string) {
    ...
    this.dataService.updateEmployeeAcceptCount(customid, this.empAcceptCount).subscribe(
      ...
      error => {
        ...
        // Call the service to update emp_accept_count in the database
        this.dataService.updatecnc01runtime(customid, this.runtime).subscribe(
          response => {
            console.log('Employee accept count updated successfully:', response);
            // Optionally, you can refresh the page or update the view here
          },
  ...
  stopAndUpdateMachineMode(machineName: string, processType: string, partId: string, processName: string, uniqid: any) {
